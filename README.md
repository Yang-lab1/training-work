...
AGNES_BASE_URL=
AGNES_MODEL=
AGNES_CHAT_PATH=/chat/completions
```
...
- AGNES 已实现 OpenAI-compatible chat completion 适配，优先用于文本反馈和公司知识包。设置 `AI_PROVIDER=agnes`，并提供 `AGNES_API_KEY`、`AGNES_BASE_URL` 后启用；如 AGNES 的路径不是 `/chat/completions`，用 `AGNES_CHAT_PATH` 覆盖。
- 如果只希望“公司资料增强/公司知识包”走 AGNES，而其他文本任务继续走 DeepSeek，请保持 `AI_PROVIDER=deepseek`，并额外设置 `COMPANY_KNOWLEDGE_PROVIDER=agnes`。
- 豆包、OpenAI、Gemini 已预留；未启用或未实现时回退 Mock。
